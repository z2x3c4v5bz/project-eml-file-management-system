...
def env(tmp_path):
    ...
    return cfg, db, processor, tmp_path
...
class TestProcessSimple:
    ...
    def test_archive_folder_named_after_subject(self, env):
        ...
        src = _copy("simple.eml", tmp / "watch")
        proc.process(src)
        archive = pathlib.Path(cfg.archive_root)
        subject_folders = list(archive.iterdir())
...
class TestDuplicateDetection:
    ...
    def test_duplicate_goes_to_duplicates_folder(self, env):
        ...
        dup_folder = pathlib.Path(cfg.archive_root) / cfg.duplicates_folder
